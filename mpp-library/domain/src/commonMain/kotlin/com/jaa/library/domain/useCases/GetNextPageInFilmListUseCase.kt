...
import com.jaa.library.domain.repository.filmList.FilmListRepositoryInterface.Companion.DATA_SOURCE_ROW_LIMIT
import com.jaa.library.domain.repository.filmList.FilmListRepositoryInterface.Companion.DATA_SOURCE_ROW_TITLE
...
class GetNextPageInFilmListUseCase(
    ...
    suspend fun execute(wifiActive:Boolean, listener:GetNextPageInFilmListListener) {
        if (!gettingList) {
            gettingList = true
            filmListRepository.getFilmListWithPage(offset, DATA_SOURCE_ROW_LIMIT, DATA_SOURCE_ROW_TITLE, wifiActive, object :
                FilmListRepositoryInterface.OnGetListListener {
